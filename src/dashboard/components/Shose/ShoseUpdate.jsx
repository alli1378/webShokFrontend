...
import {getUsersMain,getAllCategory,getSpecialShose, handleShoseUpdate} from '../../../actions/Dashboard'
import {} from '../../../actions/Dashboard'
// import'../../../../public/boxicons-2.0.9/css/boxicons.css'
function ShoseUpdate({match,shose,history}) {
  ...
  // console.log(user.is_superuser);
    ...
    useEffect(() => {
        ...
        for (let index = 0; index < shose.length; index++) {
            if(shose[index].id==match.params.id) {   
                const elem=shose[index]
                setsizeshose(elem.sizeshose)
                setproduct(elem.product.id)
                setseller_2(elem.seller_2.id)
                setcount(elem.count)
                setprice(elem.price)
    ...
    const handleSubmit=event=>{
        ...
        try{
            ...
            const data={sizeshose,count,price}
            dispatch(handleShoseUpdate(data,match.params.id,history))   
            console.log(data);
